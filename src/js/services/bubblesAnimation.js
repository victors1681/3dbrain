...
class BubblesAnimation {
  ...
  getBubblesSelected (bubbles, subsystem) {
    const memories = this._mainBrain.memories
    let bubbleList = []
    this.memorySelected.forEach(m => {
      ...
      const parent = this._mainBrain.particlesSystem.particles

      this._mainBrain.font.makeTextSprite(_.capitalize(m), parent, new THREE.Vector3(x, altitude - 15, z), 4)
  ...
  initAnimation (selector) {
    const scene = this._mainBrain.scene
    const camera = this._mainBrain.camera
    const memories = this._mainBrain.memories
    ...
    for (var i = 0; i < particles - (this.memorySelected.length * 3); i++) {
  ...
  updateSubSystem (subsystemPayload) {
    ...
    this._mainBrain.font.removeText()
